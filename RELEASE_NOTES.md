...
- **Proactive input watcher.** While an interactive session is open and the agent is idle, a background watcher polls it (via a non-advancing `peek`, so it never steals the agent's output). When output stays quiet *and* prompt-shaped, it makes one pure (tool-less) LLM call — given the terminal contents **and** the task context — that plays the agent's role and decides: **auto-answer** when the choice is obvious, safe and reversible (and tells the user what it did), or **ask the user** otherwise. Destructive/irreversible prompts (`rm`, overwrite, force-push, deploy, sudo, payments…) and a runaway answer→prompt→answer loop are hard-blocked from auto-answering. On by default; `CLAW_TERMINAL_WATCH=0` disables it, `CLAW_TERMINAL_WATCH_AUTO=0` makes it always ask rather than auto-answer.
