            ...
            "sections": [
                {
                    ...
                    "fields": [
                        ...
                        _field("provider_keys.xai_headers", "xAI Extra Headers",
                               type="tags", hint="Custom HTTP headers (Header-Name: value). When set, API key is ignored."),
                        _field("provider_keys.openrouter", "OpenRouter"),
                        _field("provider_keys.openrouter_headers", "OpenRouter Extra Headers",
                               type="tags", hint="Custom HTTP headers (Header-Name: value). When set, API key is ignored."),
            ...
            "sections": [
                {
                    ...
                    "fields": [
                        _field("model.provider", "Provider", type="select",
                               options=["openai", "anthropic", "ollama", "gemini", "xai", "openrouter", "litert"]),
        ...
        # ── 8. Features ──────────────────────────────────────────
        {
            ...
            "sections": [
                ...
                },
                {
                    "id": "insights",
                    "title": "Insights",
                    "description": "Auto-extracted facts, contacts, decisions, and deadlines.",
                    "fields": [
                        _field("insights.enabled", "Enabled", type="toggle"),
                        _field("insights.auto_extract", "Auto-extract", type="toggle"),
                        _field("insights.inject_in_context", "Inject in context", type="toggle"),
                        _field("insights.max_items_in_prompt", "Max items in prompt", type="number", min=1, max=50),
                        _field("insights.extraction_interval_messages", "Extraction interval (messages)", type="number", min=1, max=100),
                        _field("insights.extraction_cooldown_seconds", "Extraction cooldown (seconds)", type="number", min=10, max=3600),
                        _field("insights.max_insights", "Max insights", type="number", min=10, max=10000),
                        _field("insights.db_path", "Database path", type="text"),
                    ],
                },
                {
                    "id": "datastore",
                    "title": "Datastore",
                    "description": "User-facing relational datastore for structured data.",
                    "fields": [
                        _field("datastore.enabled", "Enabled", type="toggle"),
                    ],
                },
                {
                    "id": "nervous_system",
                    "title": "Nervous System",
                    "description": "Autonomous pattern-recognition and memory synthesis (dreaming).",
                    "fields": [
                        _field("nervous_system.enabled", "Enabled", type="toggle"),
                        _field("nervous_system.auto_dream", "Auto-dream", type="toggle"),
                        _field("nervous_system.inject_in_context", "Inject in context", type="toggle"),
                        _field("nervous_system.max_items_in_prompt", "Max items in prompt", type="number", min=1, max=20),
                        _field("nervous_system.dream_interval_messages", "Dream interval (messages)", type="number", min=1, max=100),
                        _field("nervous_system.dream_cooldown_seconds", "Dream cooldown (seconds)", type="number", min=30, max=7200),
                        _field("nervous_system.max_intuitions", "Max intuitions", type="number", min=10, max=10000),
                        _field("nervous_system.min_confidence_for_context", "Min confidence for context", type="range", min=0, max=1, step=0.05),
                        _field("nervous_system.decay_after_days", "Decay after (days)", type="number", min=1, max=365),
                        _field("nervous_system.decay_rate_per_day", "Decay rate per day", type="range", min=0, max=0.5, step=0.01),
                        _field("nervous_system.delete_threshold", "Delete threshold", type="range", min=0, max=0.5, step=0.01),
                        _field("nervous_system.allow_public", "Allow in public mode", type="toggle"),
                        _field("nervous_system.db_path", "Database path", type="text"),
                    ],
                },
                {
                    "id": "sister_session",
                    "title": "Sister Session",
                    "description": "Proactive autonomous work triggered by insights and dreams.",
                    "fields": [
                        _field("sister_session.enabled", "Enabled", type="toggle"),
                        _field("sister_session.auto_trigger_intuitions", "Auto-trigger from intuitions", type="toggle"),
                        _field("sister_session.auto_trigger_deadlines", "Auto-trigger from deadlines", type="toggle"),
                        _field("sister_session.min_intuition_confidence", "Min intuition confidence", type="range", min=0, max=1, step=0.05),
                        _field("sister_session.min_intuition_importance", "Min intuition importance", type="number", min=1, max=10),
                        _field("sister_session.max_tasks_per_hour", "Max tasks per hour", type="number", min=1, max=20),
                        _field("sister_session.max_tokens_per_day", "Daily token budget", type="number", min=10000, max=500000, step=10000),
                        _field("sister_session.default_token_budget", "Per-task token budget", type="number", min=5000, max=50000, step=1000),
                        _field("sister_session.max_queued_tasks", "Max queued tasks", type="number", min=1, max=50),
                        _field("sister_session.max_iterations", "Max agent iterations", type="number", min=1, max=10),
                        _field("sister_session.briefing_inject_in_context", "Inject briefings in context", type="toggle"),
                        _field("sister_session.max_briefings_in_context", "Max briefings in context", type="number", min=1, max=10),
                        _field("sister_session.allow_public", "Allow in public mode", type="toggle"),
                        _field("sister_session.db_path", "Database path", type="text"),
                    ],
                },
            ],
        },
            ...
            "sections": [
                {
                    ...
                    "fields": [
                        ...
                        _field("deep_memory.embedding_dims",
                               "Embedding dimensions (0 = auto-detect)", type="number"),
                        _field("deep_memory.min_score",
                               "Relevance floor (0-1)", type="number"),
