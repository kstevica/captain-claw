...
- **One continuous artifact, one writer — or ordered, non-overlapping ranges.** A single continuous work that must read as one piece (a manuscript, a long report body, a narrative) is easy to break by splitting it across two parallel writers who then overlap or leave a gap. Prefer ONE writer subtask for it. If it genuinely must be split, give each part a hard, NON-OVERLAPPING range (e.g. chapters 1–5, chapters 6–10), make each later part `depends_on` the part before it so it starts from a frozen, complete predecessor, and never put two subtasks with the same `owner_archetype_id` on the same range. The reporter still assembles; the ranges keep the parts from colliding.
