...
import { Cpu, Play, Square, RotateCcw, Trash2, ScrollText, ChevronUp, MessageSquare, Loader2, FolderOpen, Database, Pencil, Check, X, Copy, MoreVertical, Minimize2, Maximize2, Settings, Leaf, Download, Upload, Brain, Inbox, ShieldAlert, Eraser } from 'lucide-react'
...
export function ProcessCard({ process: proc, onBrowseFiles, onDragStart, isDragging }: {
  ...
  const toggleLogs = async () => {
    ...
    setLogsLoading(true)
    try {
      const result = await getProcessLogs(proc.slug, LOG_TAIL_LINES)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const result = await getProcessLogs(proc.slug, LOG_TAIL_LINES, cursor)
        logCursorRef.current = result.byte_offset || 0
        if (result.logs) {
